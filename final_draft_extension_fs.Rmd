...
kable(imp_felm1_sum_table_vars, "latex",
      caption = "\\textbf{Effect of Segregation on Overall per Capita City Expenditures: Direct General Expenditure per Capita}",
      booktabs = T) %>%
    row_spec(0, bold = TRUE) %>%
  footnote(general = c("This table shows the results of Trounstine's first model for the effect of segregation on per capita",
  ...
  "has decreased to essentially zero and has become statistically insignificant. The very large P-Value may indicate there is",
  "a larger difference between the two models. The coefficient on diversity has increased in magnitude and become",
...
```{r results_printing_fs_impute_regressions_3, results = "asis", echo=FALSE, eval = FALSE, invlude = FALSE}
...
pop_data_over_baqir <- tibble(
  log_pop = complete(imp_fs_pop_overbaqir,1)$logpop,
  Analysis = "Imputed, pop. > 25k"
)

pop_data_under_baqir <- tibble(
  log_pop = complete(imp_fs_pop_under_baqir, 1)$logpop,
  Analysis = "Imputed, pop. < 25k"
)

pop_data_imp_fs <- tibble(
  log_pop = complete(imp_fs, 1)$logpop,
...
```{r city_pops_histogram, echo = FALSE}
...
pop_hist_data %>%
  ...
  ggtitle("Histograms of Log Populaiton of City Observations Included in the Various\nRegression Analyses") +
  labs(caption = "This figure makes clear the differences in city size (in terms of population) among the different analyses. It is\nclear that the listwise deletion of observations with missing values resulted in a sample that almost entirely\nconsisted of larger cities in Trounstine (2016). The distribution of city population in Trounstine (2016) more\nclosely resembles the distribution of the original sample with a greater than 25,000 population cutoff imposed\nthan the original sample. The red lines indicate the median log population in each sample.") +
  ylab("Count") +
  xlab("Log Population") +
  theme_bw() +
...
```{r calc_coeff_sds_original_fs_impute, include = FALSE, eval = FALSE}
...
model_coeff <- tibble(
  variable = c("H_citytract_NHW_i", "diversityinterp", 
               "H_citytract_NHW_i", "diversityinterp", 
               "H_citytract_NHW_i", "diversityinterp",
               "H_citytract_NHW_i", "diversityinterp"),
  estimate = c(imp_felm1_sum[1,2], imp_felm1_sum[2,2],
               imp_under_baqir_1_sum[1,2], imp_under_baqir_1_sum[2,2],
  ...
  std_error = c(imp_under_baqir_1_sum[1,3], imp_under_baqir_1_sum[2,3],
                imp_felm2_sum[1,3], imp_felm2_sum[2,3],
