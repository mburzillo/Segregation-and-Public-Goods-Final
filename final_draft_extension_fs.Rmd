...
```{r over_baqir_fit, include = FALSE}
# fit multiple imputed datasets
...
miss_var_summary(complete(imp_fs_pop_overbaqir))
...
```{r population_under_Baqir_fitting, include = FALSE}
...
```{r histograms_city_pop_in_specifications}
...
fin_seg %>%
  filter(fin_seg$logpop == 0)

fin_seg %>%
  filter(populationinterp < 25000)
